...
import { LaporanSewa, PaketSewa } from "../../../electron/@types/models";
...
const metodeIsiTanggalSewa = ref<"sekarang" | "manual">("manual");

// Form
const form = reactive({
  nama_penyewa: props.dataForUpdate.nama_penyewa,
  alamat_penyewa: props.dataForUpdate.alamat_penyewa,
  kontak_penyewa: props.dataForUpdate.kontak_penyewa,
  tanggal_sewa: {
    tahun: props.dataForUpdate.tanggal_sewa.split(" ")[0].split("-")[0],
    bulan: props.dataForUpdate.tanggal_sewa.split(" ")[0].split("-")[1],
    tanggal: props.dataForUpdate.tanggal_sewa.split(" ")[0].split("-")[2],
    jam: props.dataForUpdate.tanggal_sewa.split(" ")[1].split(":")[0],
    menit: props.dataForUpdate.tanggal_sewa.split(" ")[1].split(":")[1],
    detik: props.dataForUpdate.tanggal_sewa.split(" ")[1].split(":")[2],
  },
  paket_sewa: {
    id: 0,
    nama_paket: props.dataForUpdate.paket_sewa,
    harga: props.dataForUpdate.total_bayar,
  },
  status: 0,
});
const rules = ref({
...
async function onSubmit() {
  ...
  try {
    await backendFetch(
      laporanSewaCrudChannelNames.UPDATE,
      Object.assign(
        {},
        {
    ...
    $toast.success(
      `Berhasil mengubah data penyewaan id ${props.dataForUpdate.id}`,
    );
    emits("submit");
  } catch (error: any) {
...
onMounted(() => {
  form.status = props.dataForUpdate.status;
});
</script>
<template>
  <v-card :title="`Ubah data sewa id ${dataForUpdate.id}`">
    <v-card-subtitle>
    ...
    >
      ...
      <div class="d-flex justify-end gap-2 mt-2">
        ...
        <v-btn type="submit" variant="tonal" color="warning">OK</v-btn>
      </div>
    </v-form>
  </v-card>
</template>
